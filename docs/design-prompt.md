...
- 支持**并发度控制**：限制最大并行 worker 数，避免资源耗尽；
...
| **Engine** | DAG 的执行引擎，负责拓扑排序、并发调度（worker pool）、结果收集。 |
| **Scheduler** | 从 Engine 中拆出的调度策略模块，负责就绪节点的优先级排序（无锁，主 goroutine 独占），方便后续替换调度策略。 |
...
├── scheduler.go        // 调度策略：优先级、就绪队列管理（无锁，主 goroutine 独占）
├── workerpool.go       // 固定数量 worker 池，替代 per-node goroutine 创建
│
│── Data（数据层）───────────────────────────────
